...
{
    map<string, map<string, double>> dm; // Change int to double
    ...
    while (getline(file, line))
    {
        ...
        {
            if (dist.length() != 0)
                dm[point][points[idx]] = stod(dist); // Change stoi to stod for double values
    ...
    string pt1, pt2;
    double min_dist = DBL_MAX; // Use DBL_MAX for double
